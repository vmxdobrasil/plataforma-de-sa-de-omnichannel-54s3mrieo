...
| **Memed (Prescrição Digital)**                            | Prescrição médica digital inteligente com catálogo ANVISA, tipos simples/controlados e envio multicanal. | `src/components/clinic/EmbeddedMemedPrescription.tsx`<br>`src/components/clinic/MemedConnectionCard.tsx`<br>`src/services/memed.ts`<br>`pocketbase/migrations/0113_memed_integration_and_prescriptions.js`<br>`pocketbase/migrations/0115_post_prescription_loop_fields.js`<br>`pocketbase/hooks/memed_oauth.js`<br>`pocketbase/hooks/memed_audit_hooks.js`<br>`src/pages/MyPrescriptions.tsx` | **Loop pós-prescrição 100% fechado e validado (v0.0.346)**. Editor embutido (simples, controlados azul B1/B2 e amarelo A1/A2/A3, exames, atestados), persistência em `prescriptions` com dados Memed (`memed_prescription_id`, `document_validation_url`, `prescription_type`, `status`, `signed_at`), vinculação ao prontuário em `health_records`, envio SMS/e-mail com registro em `messages`, tela do paciente `/my-prescriptions` (QR code de validação, seletor de dependentes, impressão), reenvio com permissão restrita (autor, diretoria, admin), auditoria em `/admin/audit`. | Pré-cadastro de parceria **JÁ REALIZADO** no formulário `memed.com.br/parceiro-software`; aguardando contato do especialista para credenciais de sandbox; OAuth/onboarding de conexão (aba "Prescrição Memed" no painel do profissional, 3 passos) e prescrição embutida já implementados operando em modo estrutural/sandbox. |
| **CFM / Conselho Federal de Medicina (Validação de CRM)** | Verificação e validação do CRM e situação cadastral do médico (ativo/suspenso/cassado).                  | `src/services/crm-validation.ts`<br>`pocketbase/migrations/0116_create_professional_verifications.js`<br>`pocketbase/hooks/crm_validation.js`<br>`src/pages/AdminVerification.tsx`<br>`docs/PETICIONAMENTO_CFM_WEBSERVICE.md`                                                                                                                                                                  | **Implementado com Fallback Resiliente**. Validação de formato em tempo real (máscara, normalização, 27 UFs), verificação de regularidade (aceita SOMENTE "ativo"), checagem de divergência de nome, preenchimento de especialidade, auditoria em `audit_logs` e fila de validação manual no `/admin/verification`.                                                                                                                                                                                                                                                                      | Credenciamento oficial CFM via SEI-Medicina normatizado pela Res. CFM 2.129/15 e 2.309/22 em tramitação (rascunho pronto em `docs/PETICIONAMENTO_CFM_WEBSERVICE.md`); chave de API de terceiros (Infosimples) configurável via secret `CFM_API_KEY`.                                                                           |
...
3. **Assinatura Digital ICP-Brasil & Memed:** O loop pós-prescrição Memed já está 100% concluído na plataforma (v0.0.346), com receitas simples e de controle especial (azul e amarelo), vínculo ao prontuário, portal do paciente com QR Code de validação do CFM/ITI e auditoria em `/admin/audit`. A assinatura ICP-Brasil em nuvem opera integrada ao fluxo Memed assim que as credenciais definitivas de parceiro de software forem ativadas pelo CFM/Memed.
...
│   └── PETICIONAMENTO_CFM_WEBSERVICE.md # Processo SEI-Medicina para credenciamento no Webservice CFM
├── pocketbase/
│   ├── hooks/                          # Serverless pb_hooks (Goja JSVM / PocketBase)
│   └── migrations/                     # Migrações incrementais do banco de dados (0001 a 0116)
