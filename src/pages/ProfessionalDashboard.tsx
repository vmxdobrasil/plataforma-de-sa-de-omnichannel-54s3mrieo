    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold">R$ 4.2k</h3>
            </div>
          ...
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <h3 className="text-xl font-bold">
                {activeAppt
                  ? `${format(new Date(activeAppt.dateTime), 'HH:mm')} - ${activeAppt.expand?.patient_id?.name?.split(' ')[0]}`
                  : 'Livre'}
              </h3>
            </div>
      ...
      >
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle className="text-lg">Agenda Recente e Futura</CardTitle>
          </CardHeader>
              ...
              {appointments.map((apt) => (
                ...
                >
                  ...
                  <Avatar className="h-10 w-10 border">
                    <AvatarImage
                      src={`https://api.dicebear.com/7.x/notionists/svg?seed=${apt.expand?.patient_id?.name}`}
                    />
                    <AvatarFallback>{apt.expand?.patient_id?.name?.[0]}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 overflow-hidden">
                    <p className="font-semibold text-sm truncate">{apt.expand?.patient_id?.name}</p>
                    <p className="text-xs text-muted-foreground truncate">{apt.type}</p>
                  </div>
                  {apt.type === 'Online' && <Video className="h-4 w-4 text-blue-500 shrink-0" />}
                </div>
              ))}
            ...
            <Tabs defaultValue="notes" className="w-full mt-2">
              ...
              <TabsContent value="notes" className="space-y-4 mt-4">
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" disabled={!activeAppt}>
                    Salvar Rascunho
                  </Button>
                  <Button disabled={!activeAppt}>Assinar e Fechar</Button>
                </div>
              </TabsContent>
